...
def get_all_summary(csv_dir):
    ...
    forsale_files = [file for file in files if file.startswith('forsale_') and file.endswith('.csv')]
    print(forsale_files)
    df = pd.DataFrame()
...
def get_summary_df(csv_file):    
    ...
    summary_by_zipcode = df.groupby('zipcode').agg(
    ...
    overall_summary = pd.DataFrame({
        ...
        'collectedDate': pd.to_datetime(collectedDate, format='%Y-%m-%d')  # Use the same date for the overall summary
    }, index=[0])
    
    # Append the overall summary to the summary DataFrame
    summary_by_zipcode = pd.concat([summary_by_zipcode, overall_summary], ignore_index=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Get summary from a given forsale csv file")
